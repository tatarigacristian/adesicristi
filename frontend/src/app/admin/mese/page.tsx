...
export default function MesePage() {
  ...
  const fetchAll = useCallback(async () => {
    const [settingsRes, guestsRes, assignRes, rsvpRes] = await Promise.all([
  ...
  const tables = useMemo(() => {
    ...
    const result: { number: number; guests: { id: number; name: string; partnerId: number | null }[] }[] = [];
    for (let i = 1; i <= settings.numar_mese; i++) {
      ...
      result.push({ number: i, guests: tableGuests });
    }
    return result;
  }, [settings, mainGuests, assignmentMap, guests]);

  // Count people (each plus_one guest counts as 2)
  function countPeople(tableGuests: { id: number; partnerId: number | null }[]) {
    return tableGuests.reduce((sum, g) => {
  ...
  const totalAssigned = tables.reduce((sum, t) => sum + countPeople(t.guests), 0);
  const totalUnassigned = unassigned.reduce((sum, g) => sum + (g.plus_one ? 2 : 1), 0);
    ...
    <div>
          ...
          <div className="bg-white rounded-xl p-5 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-5 sm:grid-cols-8 gap-2">
              {Array.from({ length: settings.numar_mese! }, (_, i) => i + 1).map((n) => {
                ...
                const count = table ? countPeople(table.guests) : 0;
                const st = getStatus(count);
                return (
                  <button
                    key={n}
                    onClick={() => assignGuest(assigning, n)}
        ...
        <div className="mt-3 max-h-[300px] overflow-y-auto space-y-1">
          {unassigned.length === 0 ? (
            <p className="text-xs text-text-muted py-4 text-center">
              {search ? "Niciun rezultat" : "Toti invitatii sunt asezati!"}
            </p>
          ) : (
            unassigned.map((g) => {
              const partner = g.partner_id ? guests.find((p) => p.id === g.partner_id) : null;
      ...
      {(() => {
        ...
        const renderTable = (table: typeof tables[0]) => {
          const count = countPeople(table.guests);
