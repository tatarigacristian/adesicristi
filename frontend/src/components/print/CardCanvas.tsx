...
export interface CardGuestData {
  id: number;
  nume: string;
  prenume: string;
...
export interface CardPartnerData {
  nume: string;
  prenume: string;
}

export interface CardWeddingSettings {
  nume_mire: string;
  nume_mireasa: string;
...
function CardEventIcon({ icon, color }: { icon: "church" | "transport" | "party"; color: string }) {
  switch (icon) {
    case "church": return <Church size={14} weight="duotone" color={color} />;
    case "transport": return <Bus size={14} weight="duotone" color={color} />;
    case "party": return <Champagne size={14} weight="duotone" color={color} />;
  }
}

export function CardFront({ guest, partner, settings, qrDataUrl }: { guest: CardGuestData; partner: CardPartnerData | null; settings: CardWeddingSettings; qrDataUrl: string }) {
  ...
  const dateDisplay = settings.ceremonie_data ? formatDate(settings.ceremonie_data) : "4 Iulie 2026";
      ...
      <div className="card-inner" style={{ display: "grid", placeContent: "center", justifyItems: "center", padding: "0.3cm 0.8cm", gap: 0 }}>
        ...
        <div style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "center", width: "100%", gap: "0.5cm" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1 }}>
            ...
            <p className="card-label" style={{ fontSize: "0.38rem", marginTop: "0.08cm", fontWeight: 600 }}>Scanați codul</p>
          </div>
        </div>
...
export function CardBack({ guest, partner, settings }: { guest: CardGuestData; partner: CardPartnerData | null; settings: CardWeddingSettings }) {
  const mireasa = settings.nume_mireasa || "Ade";
  const mire = settings.nume_mire || "Cristi";
  const text = settings.color_text || "#344b30";
  const accent = settings.color_button || "#7f9f84";
  ...
  const introText = guest.intro_short ? guest.intro_short : getDefaultIntroShort(audience);

  const events = [
    { icon: "church" as const, label: settings.ceremonie_ora ? `ora ${settings.ceremonie_ora}` : "", desc: settings.ceremonie_descriere || "Ceremonie", address: settings.ceremonie_adresa || "" },
    { icon: "transport" as const, label: settings.transport_ora ? `ora ${settings.transport_ora}` : "", desc: settings.transport_descriere || "Transport", address: settings.transport_adresa || "" },
    { icon: "party" as const, label: settings.petrecere_ora ? `ora ${settings.petrecere_ora}` : "", desc: settings.petrecere_descriere || "Petrecere", address: settings.petrecere_adresa || "" },
  ].filter((e) => e.label);
...
export function buildCardStyles(s: CardWeddingSettings | null): string {
  ...
  return `
    ...
    .card-page-root .card-label { font-family: var(--font-montserrat), 'Montserrat', sans-serif; font-size: 0.35rem; letter-spacing: 0.3em; text-transform: uppercase; color: ${button}; font-weight: 800; margin-bottom: 0.08cm; }
    .card-page-root .card-couple-names { font-family: var(--font-script), 'Alex Brush', cursive; font-size: 1.5rem; color: ${text}; font-weight: 400; line-height: 1.1; margin-bottom: 0.1cm; }
    .card-page-root .card-qr { margin-bottom: 0.08cm; }
    .card-page-root .card-qr img { width: 1.5cm; height: 1.5cm; display: block; border-radius: 2px; }
    ...
    .card-page-root .card-greeting { font-family: var(--font-cormorant), 'Cormorant Garamond', serif; font-size: 0.5rem; color: ${text}; font-weight: 800; margin-top: 0; margin-bottom: 0.05cm; }
    .card-page-root .card-nasi { font-family: var(--font-cormorant), 'Cormorant Garamond', serif; font-size: 0.38rem; color: ${muted}; font-weight: 700; font-style: italic; margin-bottom: 0.08cm; }
    .card-page-root .card-message { font-family: var(--font-cormorant), 'Cormorant Garamond', serif; font-size: 0.42rem; color: ${lightenHex(text, 30)}; font-weight: 600; font-style: italic; line-height: 1.6; text-align: center; margin-bottom: 10px; }
    .card-page-root .card-events-row { display: flex; gap: 0.35cm; margin-bottom: 0.12cm; }
    .card-page-root .card-event-item { display: flex; flex-direction: column; align-items: center; gap: 0.04cm; flex: 1; }
    .card-page-root .card-event-time { font-family: var(--font-montserrat), 'Montserrat', sans-serif; font-size: 0.3rem; font-weight: 800; color: ${text}; letter-spacing: 0.05em; }
    .card-page-root .card-event-address { font-family: var(--font-montserrat), 'Montserrat', sans-serif; font-size: 0.24rem; font-weight: 700; color: ${muted}; text-align: center; line-height: 1.3; white-space: nowrap; }
    .card-page-root .card-back-footer { text-align: center; }
    .card-page-root .card-back-names { font-family: var(--font-script), 'Alex Brush', cursive; font-size: 0.85rem; font-weight: 700; color: ${text}; margin: 0; }
  `;
}
